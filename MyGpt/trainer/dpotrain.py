...
# 用来衡量相对于标签的一个差距
def logits_to_log_probs(logits, label_ids):
    # logits (batch, seq, vocab_size)
    # label_ids (batch, seq)
    logits = F.log_softmax(logits, dim = 2)
    # torch.gather 的要求除了dim指定以外的其他维度必须完全一样
    # rank & dim   -> value
    # 因此这里label_ids必须unsqueeze(2)
    # 返回值就是 (batch, seq) dtype变成了torch.float
    return torch.gather(logits, dim=2, index=label_ids.unsqueeze(2)).squeeze(-1)

def dpo_loss(ref_log_probs, policy_log_probs, mask, beta):
    # 维度变为了 batch,
    ref_log_prob = torch.mean(ref_log_probs * mask, dim=1)
    policy_log_prob = torch.mean(policy_log_probs * mask, dim=1)

    batch_size = ref_log_prob.shape[0]
    chosen_ref_log_prob = ref_log_prob[:batch_size // 2]
    rejected_ref_log_prob = ref_log_prob[batch_size // 2 :]
    chosen_policy_log_prob = policy_log_prob[:batch_size // 2]
    rejected_policy_log_prob = policy_log_prob[batch_size // 2 :]

    # 这里用来衡量好坏答案的程度
    ref_logratio = chosen_ref_log_prob - rejected_ref_log_prob
    policy_logratio = chosen_policy_log_prob - rejected_policy_log_prob
...
if __name__ == "__main__":
    ...
    train_ds = DPODtaSet(args.max_seq_len, tokenizer, args.data_path)
